...
def get_current_mac(interface):
    try:
        ...
        mac_Address_res = re.search(r"\w\w:\w\w:\w\w:\w\w:\w\w:\w\w", str(ifconfig_result))
        if mac_Address_res:
            return mac_Address_res.group(0)
        else:
            print("[-] Could not find mac address")
    except subprocess.CalledProcessError:
        print("[-] Error fetching. Make sure given interface is valid or present")
...
if __name__ == '__main__':
    ...
    elif options.mode == '1' and current_mac is not None:
        change_mac(options.interface, options.new_mac)
        current_mac = get_current_mac(options.interface)
        if current_mac == options.new_mac:
            print(f'[+] MAC address succesfully changed to {current_mac}')
